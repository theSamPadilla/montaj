...
- **Profile snapshot in project.json** — When `profile` is set, project.json also contains `profileSnapshot` with `notes` (hand-curated profile-wide creative rules) and `availableAssets` (a list of `{filename, description, tags}` entries). These are pinned at project init — they reflect the profile's state at creation time, not the live profile dir. Read them as creative guidance: notes are explicit rules ("always end with bumper.mov"), availableAssets is a directory of reusable files the user has curated. The user includes specific assets via the editor side panel; included assets land in `project.assets[]` with the same shape as any other asset. **Never call the include-asset endpoint on the user's behalf without explicit instruction** — selection is human-driven.
- **`style_profile.md` vs `profileSnapshot.notes`**: `style_profile.md` is *analysis-derived* (generated by `montaj profile analyze` from observed sample videos — describes patterns); `profileSnapshot.notes` is *hand-curated rules* the user wrote that are tied to the asset library. When the two conflict, `notes` wins — it's the explicit user rule.
