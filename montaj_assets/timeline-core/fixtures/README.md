...
Fourteen committed `project.json`-shaped fixtures (`"version": "0.2"`, the
...
| `source-crop.json` (**Bug B**) | `sourceCrop` + `sourceWidth` + `sourceHeight` all present on a video item — `geometryFor` forwards all three verbatim by reference. Also the fixture used for the **Part C encode-args golden** (`expected/encode-args.source-crop.json`). SP3: the item now also carries a preview-only `proxySrc`, which is exactly what makes `expected/encode-args.source-crop.json` staying byte-identical the render-untouched proof — render keeps resolving to the master regardless of what preview picks. | none — this is the FIX (verbatim forwarding), verified working |
...
| `proxy-matrix.json` | The 2×2 matrix of `proxySrc` × `nobg_preview_src` (SP3), every row also carrying `normalizedSrc` + `normalizedInPoint`. Row `proxy-01` (`proxySrc` only) pins the alpha-safe tier: preview picks the full-source proxy and does NOT rebase (`usedNormalizedCache: false`, `inPoint` stays `6`), while render picks `normalizedSrc` rebased to `1`. Row `proxy-11` pins `nobg_preview_src` winning over `proxySrc` — precedence order is unchanged by SP3. | none — this is the FIX (proxy precedence), verified working |
