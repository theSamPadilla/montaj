...
- **Agent-first model.** New carousel projects start in `status: "pending"` — the editor opens to a "Send this to your agent" copy panel pointing at the root `SKILL.md`. The agent reads the workflow → finds the new `montaj/carousel` callable skill (`skills/carousel/SKILL.md`) → builds slides via `PUT /api/projects/{id}` → renderer produces PNGs. Once the agent flips status off `pending`, the canvas opens for direct edits (drag/resize/rotate elements, snap guides at slide center axes and edges, 90° rotation snap). Live status messages from the agent (`POST /api/projects/{id}/log`) appear in the pending screen.
